"""
Lab 04 Reference: Main demo script.
Applies all filters and decodes the steganographic message.

Run from lab04/ directory, or from lab04/src/ (auto-detects).
"""
import os
import sys

# If running from src/, change to parent so data/ paths work
...
def demonstrate_filters(img, title="surveillance_a"):
    ...
    plt.tight_layout()
    plt.savefig(f"data/{title}_filters.png", dpi=100)
    print(f"Saved: data/{title}_filters.png")
...
def demonstrate_steganography():
    ...
    axes[1].axis('off')
    plt.tight_layout()
    plt.savefig("data/stego_analysis.png", dpi=100)
    print("Saved: data/stego_analysis.png")
